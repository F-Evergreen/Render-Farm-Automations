...
from U_nuke.libs import render_file_utils


# Union mod
_thisfile = os.path.basename(__file__)
...
def setBusyCursor():
    try:
        ...
        if not app:
            return
        app.setOverrideCursor(PySide2.QtGui.QCursor(PySide2.QtCore.Qt.WaitCursor))
    except:
        pass

# Union mod (p.medrow dec '18)
...
# Union test-point print
_UTP_no = 0


def UTP(msg='', print_filename=True, line_num=''):
    global _UTP_no
    _UTP_no += 1
    filename = ''
    if print_filename:
    ...
    if line_num:
        line_num = 'line {0}: '.format(str(line_num))
    nuke.tprint('UTP {0}: {1}{2}{3}'.format(_UTP_no, filename, line_num, str(msg)))

# union mod : Rhys 26/07/2019:  Adds SOFTWARE_TEMPLATE env to job submission


def add_software_template_environment(file_handle):

    if "SOFTWARE_TEMPLATE" in os.environ and os.environ["SOFTWARE_TEMPLATE"]:

        template = os.environ["SOFTWARE_TEMPLATE"]

        env_string = "EnvironmentKeyValue0=SOFTWARE_TEMPLATE={0}\n".format(template)

        file_handle.write(env_string)


def add_disable_sgtk_init_env_var(file_handle):
    """
    By adding this env var it makes sure that the legacy init_sgtk functionality in our un_nuke's init.py doesn't run
    :param file_handle: An open file object that deadline uses to write data about the render job
    :return:
    """
    env_string = "EnvironmentKeyValue0=U_DISABLE_SGTK_NUKE_INIT=1\n"
    file_handle.write(env_string)

# --- Deadline limits ------------------------------------
#
# Union mod: PeteM Jan '20
#


def check_render_limits(limits_string, do_info_dialog=True):
    limits_string = check_nuke_render_limit(limits_string)
    limits_string = check_plugin_limits(limits_string)
    return limits_string


def check_nuke_render_limit(limits_string):
    nuke_render_limit = 'nuke render license limit'
    if nuke_render_limit not in limits_string:
        if limits_string:
            limits_string += ',' + nuke_render_limit
        else:
            limits_string = nuke_render_limit
    return limits_string


def check_plugin_limits(limits_string, plugin_name=None, remove_unused=True, do_info_dialog=True):

    # dict:
    # {
    #  "plugin name from check_plugins.plugin_names()" : "deadline limit"
    # }
    plugin_limits = {
        'rsmb': 'rsmb_render',
        'neatvideo': 'neatvideo license limit',
        'opticalflares': 'opticalflares',
        'RAFT': 'gpu_render',
        'TecoGAN': 'gpu_render',
        'RealESRGAN': 'gpu_render',
        'ABME_SV': 'gpu_render',
        'ABME_Retime': 'gpu_render',
        'NNFlowVector': 'nnflow',
        'NNCleanup': 'nnflow'
    }

    from . import check_plugins

    all_nodes = nuke.allNodes()

    dialog_msg = ''

    plugin_names = plugin_limits.keys()
    if plugin_name:
        plugin_names = [plugin_name]

    added_limits = False
    removed_limits = False

    for plug_name in plugin_names:
        render_limit = plugin_limits[plug_name]
        if plug_name:
            if (plug_name not in check_plugins.plugin_names()) or (plug_name not in plugin_limits.keys()):
                nuke.tprint("Warning: {0} check_plugin_limits(): Don't know about plugin '{1}'. Valid plugin names are {2}".format(
                    os.path.basename(__file__), str(plug_name), str(check_plugins.plugin_names())))
                continue
        is_using, node_name = check_plugins.check_plugin(plug_name, all_nodes=all_nodes)
        if is_using and (render_limit not in limits_string):
            added_limits = True
            dialog_msg += "\n'{0}' for node '{1}'\n".format(plugin_limits[plug_name], node_name)
            if limits_string:
                limits_string += ',' + render_limit
            else:
                limits_string = render_limit
        elif not is_using and (render_limit in limits_string):
            removed_limits = True
            dialog_msg += "\n'{0}'\n".format(plugin_limits[plug_name])
            if limits_string:
                limits_string = limits_string.replace(render_limit, '')
                limits_string = limits_string.replace(',,', ',')

    limits_string = limits_string.strip(',')

    if do_info_dialog and dialog_msg:
        msg = ''
        if added_limits:
            msg = 'Setting the following Deadline render limits:\n' + dialog_msg + '\n' +\
                'Disable these nodes or modify the limits in the submitter UI to change this.\n\n' +\
                'See the render wranglers for help.'
        elif removed_limits:
            msg = 'Removed the following unnecessary Deadline render limits:\n' + dialog_msg + '\n' +\
                'You can modify the limits in the submitter UI to change this.\n\n' +\
                'See the render wranglers for help.'
        if msg:
            nuke.message(msg)
...
class DeadlineDialog(nukescripts.PythonPanel):
    ...
    def __init__(self, maximumPriority, pools, secondaryPools, groups):
        ...
        width = 625 # type: int
        height = 890 # type: int #Nuke v6 or earlier UI height
        ...
        # Union mod - Write node list
        self.writeNodeList = nuke.Text_Knob("Deadline_WriteNodeList", "Write Nodes")
        self.addKnob(self.writeNodeList)
        self.writeNodeList.setTooltip("A list of write nodes in the current script.")
        ...
        self.useNukeX.setEnabled(False)
        ...
        self.continueOnError.setEnabled(False)
    ...
    def knobChanged(self, knob):
        ...
        if knob == self.selectedOnly:
            self.updateWriteNodeList()

    def setWriteNodes(self, nodes):
        self.writeNodes = nodes
    
    def updateWriteNodeList(self):
        write_node_list_str = "\n"
        for write_node in self.writeNodes:
            node_name = write_node.name()
            if write_node.parent().Class() == "WriteTank":
                node_name = write_node.parent().name()
            if not self.selectedOnly.value() or IsNodeOrParentNodeSelected(write_node):
                write_node_list_str += "{: >20}{: >20}\n".format(node_name, GetFrameList(write_node))
        self.writeNodeList.setValue(write_node_list_str)
...
def CallDeadlineCommand(arguments, hideWindow=True):
    ...
    # Union mod: Test point
    # UTP('CallDeadlineCommand()', line_num=sys._getframe().f_lineno)

    deadlineCommand = GetDeadlineCommand() # type: str

    # Union mod: Test point
    # UTP('CallDeadlineCommand(): deadlineCommand: {0}'.format(str(deadlineCommand), line_num=sys._getframe().f_lineno))
    ...
    # Union mod: Test point
    # UTP('CallDeadlineCommand(): starting subprocess with args: "{0}"'.format(str(arguments)), line_num=sys._getframe().f_lineno)
    ...
    # Union mod: Test point
    # UTP('CallDeadlineCommand(): subprocess finished.', line_num=sys._getframe().f_lineno)
    # UTP('CallDeadlineCommand(): subprocess return code: {0}'.format(proc.returncode), line_num=sys._getframe().f_lineno)
    # UTP('CallDeadlineCommand(): subprocess stdout: "{0}"'.format(output), line_num=sys._getframe().f_lineno)
    # UTP('CallDeadlineCommand(): subprocess stderr: "{0}"'.format(errors), line_num=sys._getframe().f_lineno)
...
def WriteStickySettings(dialog, configFile):
    ...
    # Union mod : don't store sticky settings in user folders
...
def WriteMachineStickySettings(configFile):
    ...
    if dialog is not None:
        try:
            ...
            # Union mod : safer to just have render threads revert to default every time
...
def ReadSceneStickySettings():
    ...
    try:
        ...
        if "Group" in root.knobs():
            sticky_value = (root.knob("Group")).value()

            value_to_set = sticky_value
            print("# Found sticky group value: {}".format(sticky_value))
            # for now not all workstations seem to fully support rendering with Nuke13+ because of Python 3
            # issues. So when the nuke major version is higher than 12 check if the the default group is
            # python3.
            if nuke.NUKE_VERSION_MAJOR > 12:
                default_group = "python3"
                if sticky_value != default_group:

                    result = nuke.ask("The sticky group setting for this scene is not set to the normal "
                                      "default of '{0}' (it is '{1}'). This may result in errors when "
                                      "rendering. Would you like to switch to "
                                      "group '{0}' now?".format(default_group, sticky_value)
                                      )
                    if result:
                        value_to_set = default_group

            DeadlineGlobals.initGroup = value_to_set  # type: ignore
            
        if "Priority" in root.knobs():
            scene_sticky_priority_value = _safe_read_scene_sticky_values("Priority", int)
            if scene_sticky_priority_value:
                DeadlineGlobals.initPriority = scene_sticky_priority_value   # type: ignore
...
def _safe_read_scene_sticky_values(knob_name, expected_type):
    """
    Sticky values may not be the correct type for what Deadline is expecting, so this helps convert sticky values to
     deadline value types
    :param knob_name: the name of the deadline sticky knob on the nuke root node
    :param expected_type: The expected value type that deadline is expecting (eg: int, float, str) etc
    :return:
    """
    ...
    if not knob:
        raise RuntimeError("No knob on script root found called '{}'".format(knob))

    knob_value = knob.value()

    # if the knob value isn't of the expected type, try and convert the knob value to the expected type
    if not isinstance(knob_value, expected_type):
        try:
            knob_value = expected_type(knob_value)
        except:
            return
    # if it is the expected type then just return it.
    return knob_value
...
def SubmitSequenceJobs(dialog, deadlineTemp, tempDependencies, semaphore, extraInfo):
    ...
    for sequence in sequences:
        ...
        for comp in comps[sequence]:
            ...
            # Union mod : remove progress dialog            
            ...
            # Union mod : make render copy of script and copy to output directory
            print("Running from SubmitSequenceJobs")
            render_script_name = render_file_utils.convert_tk_write_nodes_and_save_as_render_copy()
            copy_converted_script_to_output_dir(dialog, render_script_name)
            print("# Done converting nodes")
            ...
            # Union mod : replaced with render script
            fileHandle.write("SceneFile={}\n".format(render_script_name))
...
def SubmitJob(dialog, root, node, writeNodes, deadlineTemp, tempJobName, tempFrameList, tempDependencies, tempChunkSize, tempIsMovie, jobCount, semaphore,  extraInfo):
    ...
    # todo: this can probably be removed... but we've left it in just in case to help support legacy projects
    # union mod : Rhys 26/07/2019:  Adds SOFTWARE_TEMPLATE env to job submission
    add_software_template_environment(fileHandle)
    # Add an env var to prevent the init_sgtk from running when rendering Nuke jobs. SGTK environment is no longer
    # required now that tk-nuke-writenodes are getting converted to regular write nodes.
    add_disable_sgtk_init_env_var(fileHandle)
    ...
    # Union mod : make render copy of script and copy to output directory
    print("# Running from SubmitJob()")
    # Union mod - ensure the conversion subprocess runs one at a time to prevent sgtk init issues.
    if semaphore:
        semaphore.acquire()
    render_script_name = render_file_utils.convert_tk_write_nodes_and_save_as_render_copy()
    if semaphore:
        semaphore.release()
    copy_converted_script_to_output_dir(dialog, render_script_name)
    print("# Finished converting nodes")
    ...
    if not dialog.submitScene.value():
        ...
        fileHandle.write("SceneFile=%s\n" % render_script_name)
    # We get the revision number ({2}) now to have more control over which render exe to use.
    fileHandle.write("Version={0}.{1}v{2}\n".format(*dialog.nukeVersion))
    ...
    if dialog.separateJobs.value():
        ...
        if node.parent().Class() == "WriteTank":
            # Union mod
            # An SG Write node will be converted to a stock Write node with a matching name in the render script.
            # We need to replace the name of the embedded Write node with the name of the Write node that will be
            # in the render script.
            node_name = node.parent().fullName()
        ...
        for tempNode in writeNodes:
            if not tempNode.knob('disable').value():
                ...
                if enterLoop:
                    ...
                    #we need the fullName of the node here, otherwise write nodes that are embedded in groups won't work
                    node_name = tempNode.fullName()
                    if tempNode.parent().Class() == "WriteTank":
                        # Union mod
                        # An SG Write node will be converted to a stock Write node with a matching name in the render script.
                        # We need to replace the name of the embedded Write node with the name of the Write node that will be
                        # in the render script.
                        node_name = tempNode.parent().fullName()
                    fileHandle.write("WriteNode%s=%s\n" % (writeNodeIndex,node_name))
    ...
    # Union mod: Test point
    # UTP('SubmitJob(): Written job file', line_num=sys._getframe().f_lineno)

    # Union mod: Test point
    # UTP('SubmitJob(): Submitting job file to Deadline...', line_num=sys._getframe().f_lineno)
...
def IsNodeCrossContext(node):
    # type: (Any) -> bool
    # Union mod - detect SG Write nodes that are targetting contexts other than the one the script is saved to.
    cross_context = False # type: bool
    parent = node.parent()
    if parent.Class() == "WriteTank" and parent.knob('use_context_selector'):
        cross_context = parent.knob('use_context_selector').value()
...
def GetNodeFrameRange(node):
    ...
    if node.Class() == "SmartVector":
        ...
        # Union mod - use_limit is not an enable/disable switch. Rather it affects whether or not the node is
        # disabled outside of the specified frame range.
...
def SubmitToDeadline():
    ...
    ######################################################
    # Union mod
    # this is the entry point from the menu.py menu commmand so reset the UTP counter
    # global _UTP_no
    # _UTP_no = 0
    # nuke.tprint('\n\n=================================================')
    # nuke.tprint('Submitting render to Deadline')
    # UTP('SubmitToDeadline() START.', line_num=sys._getframe().f_lineno)
    ######################################################
    ...
    # Union mod: set busy cursor as we no longer have the task progress dialog
    setBusyCursor()
    ...
    # Union mod: Test point
    # UTP('SubmitToDeadline(): Grabbing submitter info...', line_num=sys._getframe().f_lineno)
    ...
    # Union mod : make default pool "nuke"
    # DeadlineGlobals.initPool = "none" # type: ignore
    DeadlineGlobals.initPool = "nuke" # type: ignore
    ...
    # union mod: set concurrent tasks - (deadline worker settings override this).
    # DeadlineGlobals.initConcurrentTasks = 1 # type: ignore
    DeadlineGlobals.initConcurrentTasks = 2 # type: ignore
    ...
    DeadlineGlobals.initSelectedOnly = False # type: ignore
    ...
    for node in (writeNodes + smartVectorNodes + eddyCacheNodes):
        ...
        if not IsNodeReadingOrDisabled(node):
            ...
            # Union mod - detect SG Write nodes that are targetting contexts other than the one the script is saved to.
            if IsNodeCrossContext(node):
                has_cross_context_write = True
    ...
    # Union mod : Set department after the current project
    # safeSetValue(dialog.department, DeadlineGlobals.initDepartment) # type: ignore
    try:
        safeSetValue(dialog.department, os.environ['U_PROJECT']) # type: ignore
    except:
        safeSetValue(dialog.department, "None") # type: ignore
    
    # Union mod : Set pool to "nuke"
    # safeSetValue(dialog.pool, DeadlineGlobals.initPool) # type: ignore
    pool = "nuke"
    secondary_pool = "none"
    try:
        if os.environ['U_PROJECT'].lower() in submissionInfo["Pools"]:
            pool = os.environ['U_PROJECT'].lower()
            secondary_pool = "nuke"
    except:
        pass
    safeSetValue(dialog.pool, pool) # type: ignore

    # Union mod : Set secondary pool to our secondary_pool
    # safeSetValue(dialog.secondaryPool, DeadlineGlobals.initSecondaryPool) # type: ignore
    safeSetValue(dialog.secondaryPool, secondary_pool) # type: ignore
    safeSetValue(dialog.group, DeadlineGlobals.initGroup) # type: ignore

    # Union mod : Disable priority UI field and fix value
    safeSetValue(dialog.priority, 50) # type: ignore
    ...
    # Union mod : make sure we have nuke render and plugin limits set
    # safeSetValue(dialog.limitGroups, DeadlineGlobals.initLimitGroups) # type: ignore
    safeSetValue(dialog.limitGroups, check_render_limits(DeadlineGlobals.initLimitGroups)) # type: ignore
    ...
    # Union mod : Set chunk size to 5
    # safeSetValue(dialog.chunkSize, DeadlineGlobals.initChunkSize) # type: ignore
    safeSetValue(dialog.chunkSize, 5) # type: ignore
    ...
    # Union mod : pete.
    # This seems to work doing it here. Make sure this is False
    # safeSetValue(dialog.continueOnError, DeadlineGlobals.initContinueOnError) # type: ignore
    safeSetValue(dialog.continueOnError, False) # type: ignore
    ...
    # Union mod - ensure cross-context writes are done in separate jobs. 
    dialog.separateJobs.setEnabled(len(writeNodes) + len(smartVectorNodes) > 0 or has_cross_context_write)
    ...
    # Union mod - populate write node list
    dialog.setWriteNodes([writeNode for writeNode in writeNodes if not writeNode.knob('disable').value()])
    dialog.updateWriteNodeList()
    ...
    # Union mod - force "separate jobs" and "use node range" when using cross-context publishing.
    separate_jobs_original = None
    use_node_range_original = None
    if has_cross_context_write:
        if not dialog.separateJobs.value():
            separate_jobs_original = dialog.separateJobs.value()
            safeSetValue(dialog.separateJobs, True)
        if not dialog.useNodeRange.value():
            use_node_range_original = dialog.useNodeRange.value()
            safeSetValue(dialog.useNodeRange, True)
        dialog.updateWriteNodeList()
    ...
    while not success:
        ...
        if not success:
            # Union mod - restore original settings
            if separate_jobs_original:
                safeSetValue(dialog.separateJobs, separate_jobs_original)
            if use_node_range_original:
                safeSetValue(dialog.useNodeRange, use_node_range_original)
        ...
        # Union mod - warn if doing cross-context writing without using the write node frame ranges.
        if has_cross_context_write and not (dialog.separateJobs.value() and dialog.useNodeRange.value()):
            warningMessages += 'One of your write nodes is set to write to a different asset or shot. To ensure the correct frame ranges are written for each write node, please enable "Submit Write Nodes As Separate Jobs" and "Use Node\'s Frame List".\n\n'
    ...
    semaphore = multiprocessing.Semaphore() # type: threading.Semaphore
    ...
    if len(valid_projects) > 0 and dialog.submitSequenceJobs.value():
        ...
        if dialog.separateJobs.value():
            ...
            if dialog.selectedOnly.value():
                # Union mod - filter returns a generator in py3. overwriting writeNodes means
                # future lazy evaluations of the generator return nothing. storing filter
                # contents as a list instead.
            ...
            if num > 0:
                ...
                if not nuke.ask('No render order nodes found: %s \n\n' % (nodeNames) + 
            ...
            for node in tempWriteNodes:
                ...
                if enterLoop:
                    ...
                    tempJobName = dialog.jobName.value() + " - " + node_name
                    ...
                    if dialog.separateJobDependencies.value():
                        ...
                        # Union mod - replacing threading.Thread with multiprocessing.Process so submission can be terminated
                        # if the progress bar dialog is cancelled.
                        submitThread = multiprocessing.Process(None, SubmitJob, args = (dialog, root, node, tempWriteNodes, deadlineTemp, tempJobName, tempFrameList, tempDependencies, tempChunkSize, tempIsMovie, jobCount, semaphore, extraInfo)) # type: multiprocessing.Process
                        submitThread.start()
            ...
            # Union mod: Test point
            # UTP('Spawning submission thread (1)', line_num=sys._getframe().f_lineno)
            ...
            # Union mod: Test point
            # UTP('Spawning submission thread (2)', line_num=sys._getframe().f_lineno)
    ...
    # Union mod: Test point
    # UTP('SubmitToDeadline() FINISHED.', line_num=sys._getframe().f_lineno)
    # nuke.tprint('=================================================\n\n')
...
def WaitForSubmissions(submitProcesses):
    # type: (List[Any]) -> None
    # Union mod - adding a cancellable progress bar
    num_procs = len(submitProcesses)
    task = nuke.ProgressTask("Submitting {} jobs".format(num_procs))
    task.setMessage("Submitting jobs to Deadline. This may take a while.")
    num_active = num_procs
    while num_active:
        if task.isCancelled():
            for proc in submitProcesses:
                proc.terminate()
            return

        num_active = 0
        for proc in submitProcesses:
            num_active += proc.is_alive()
        task.setProgress(int(100.*((num_procs-num_active)/float(num_procs))))
        time.sleep(0.5)
...
def copy_converted_script_to_output_dir(dialog, script_path):
    """
    Copies a nuke script into the output directory of all the write nodes that will be submitted to the farm
    :param dialog: The dialog window that contains all the submission settings selected by the user
    :param str script_path: The filepath of the script we wish to copy
    """
    write_nodes = []
    # add all Write nodes to the list
    write_nodes.extend(nuke.allNodes("Write"))
    # also need to add all WriteTank nodes to the list
    write_nodes.extend(nuke.allNodes("WriteTank"))

    for current_node in write_nodes:
        to_be_copied = True

        # We need to ensure only the rendered nodes are used in the copy - modified from the checks in submitJob()
        # Ensure the copy is only run on enabled nodes
        if current_node.knob('disable').value():
            to_be_copied = False
        # If the user has ticked the "Selected Nodes Only" option, check that the write node is selected
        # also recurses to see if any parent nodes in the script are selected as those would also be rendered
        if dialog.selectedOnly.value() and not IsNodeOrParentNodeSelected(current_node):
            to_be_copied = False
        # If the user has ticked the "Nodes With 'Read File' Enabled Only" option, check that the write node has it enabled
        if dialog.readFileOnly.value() and not current_node.knob('reading').value():
            to_be_copied = False

        if to_be_copied:
            render_file_utils.copy_script_to_node_output_path(current_node, script_path)
